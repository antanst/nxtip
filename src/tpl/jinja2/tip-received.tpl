...
{%   endif %}
{% endif %}
{% set coinval_fmt = "%.6g %s%ss" % (coin_amount, amount_prefix_long, ctb.conf.coins[a.coin].name) %}
{% if amount_prefix_short %}
{%   set coinval_fmt = coinval_fmt + " (%.6f %ss)" % (a.coinval, ctb.conf.coins[a.coin].name) %}
{% endif %}
